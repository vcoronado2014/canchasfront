...
import { getUsuarioById, updateUsuario } from 'src/services/usuario.service';
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        setRol(data.rol);
        setClubId(isSuperAdmin ? data.clubId ?? '' : user?.clubId ?? '');
      } catch (error) {
        ...
        <Card sx={{ p: 4, maxWidth: 800, mx: 'auto' }}>
          <Stack spacing={3}>
            ...
            <Box display="grid" gridTemplateColumns={{ xs: '1fr', sm: '1fr 1fr' }} gap={2.5}>
              ...
              <TextField fullWidth label="Contraseña (dejar vacío para mantener)" type="password" value={password} onChange={(e) => setPassword(e.target.value)} />
            </Box>
